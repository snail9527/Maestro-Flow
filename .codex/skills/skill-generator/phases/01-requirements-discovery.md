...
```javascript
const basicInfo = await request_user_input({
  questions: [
    {
...
```javascript
const modeInfo = await request_user_input({
  questions: [
    {
...
if (executionMode === "sequential") {
  const phaseInfo = await request_user_input({
    questions: [
      {
...
if (executionMode === "autonomous") {
  const actionInfo = await request_user_input({
    questions: [
      {
...
```javascript
const toolsInfo = await request_user_input({
  questions: [
    {
      ...
      multiSelect: true,
      options: [
        { label: "User Interaction (request_user_input)", description: "Need to dialog with user" },
