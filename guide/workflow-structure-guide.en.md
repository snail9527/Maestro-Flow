...
| `roadmap.md` | Milestone/phase roadmap (created by the `roadmap` step) | Milestone list, success criteria, dependencies, Phase Progress Table |
...
| Type | ID Prefix | Scope | Producer |
|------|-----------|-------|----------|
| analyze | ANL-{NNN} | phase, adhoc, standalone | `analyze` step |
| plan | PLN-{NNN} | phase, adhoc | `plan` step |
| execute | EXC-{NNN} | phase | `execute` step |
| verify | VRF-{NNN} | phase, milestone | `execute` (E2.7) |
| review | REV-{NNN} | phase | `review` step |
| debug | DBG-{NNN} | phase, standalone | `debug` step |
| test | TST-{NNN} | phase | `test` step |
| brainstorm | BRN-{NNN} | adhoc | `brainstorm` step |
| collab | CLB-{NNN} | adhoc | `collab` step |
