...
Generate the SKILL.md orchestrator file from workflowConfig, applying all coordination patterns (progressive loading, track_tasks, data flow, conditional execution).
...
- Build track_tasks attachment/collapse patterns from phase definitions
...
function convertCommandFrontmatter(commandFm, config) {
  return {
    ...
    // "invoke_skill(*), track_tasks(*), read_file(*)" → "Task, ask_user, track_tasks, Read, Write, Edit, Bash, Glob, Grep, Skill"
  };
}
...
function expandToolWildcards(toolsStr) {
  const expanded = toolsStr
    ...
    .replace(/track_tasks\(\*\)/g, 'track_tasks')
    .replace(/Read\(\*\)/g, 'Read')
    .replace(/Bash\(\*\)/g, 'Bash')
    .replace(/Glob\(\*\)/g, 'Glob')
    .replace(/Grep\(\*\)/g, 'Grep')
  ...
  const baseTools = ['Task', 'ask_user', 'track_tasks', 'Read', 'Write', 'Edit', 'Bash', 'Glob', 'Grep'];
...
## Step 2.6: Generate track_tasks Pattern

```javascript
function generateTodoWritePattern(config) {
  let section = '## track_tasks Pattern\n\n';
  ...
  section += '   - Sub-tasks are **attached** to orchestrator\'s track_tasks\n';
  ...
  // Generate track_tasks examples for phases with sub-tasks
...
1. **Start Immediately**: First action is track_tasks initialization, second action is Phase 1 execution
...
5. **Track Progress**: Update track_tasks dynamically with task attachment/collapse pattern
...
When the skill has configurable behaviors (auto mode, force options, etc.), generate the ask_user-based preference collection section for SKILL.md:
...
function generateInteractivePreferenceCollection(config) {
  ...
  section += 'Collect workflow preferences via ask_user before dispatching to phases:\n\n';
  section += '```javascript\n';
  section += 'const prefResponse = ask_user({\n';
  section += '  questions: [\n';
...
function assembleSkillMd(config, sourceContent) {
  ...
  write_file(`${skillDir}/SKILL.md`, skillMdContent);
}
```
...
- **track_tasks**: Mark Phase 2 completed, Phase 3 in_progress
