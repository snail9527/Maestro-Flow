...
const orchestratorContent = read_file(orchestratorPath);
...
const subCommandPaths = skillCalls.map(call => {
  const parts = call.replace('invoke_skill(skill="', '').replace('"', '').split(':');
...
//   "Phase 1: ..." → first invoke_skill() call
//   "Phase 2: ..." → second invoke_skill() call
...
| track_tasks Pattern | Attachment/collapse examples | SKILL.md track_tasks Pattern |
...
const reqContent = read_file(requirementsPath);
...
const existingSkill = read_file(skillPath);

// Scan for phase files
const existingPhases = find_files(`${skillDir}/phases/*.md`);
...
const workflowConfig = {
  ...
  allowedTools: ["Agent", "ask_user", "track_tasks", "Read", "Write", "Edit", "Bash", "Glob", "Grep", "Skill"],
  ...
  // Features
  features: {
    hasAutoMode: true,                  // Interactive preference collection (ask_user)
