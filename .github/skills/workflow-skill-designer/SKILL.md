...
allowed-tools: delegate_subagent, ask_user, track_tasks, read_file, write_file, edit_file, shell, find_files, search
---
...
├── SKILL.md                    # Orchestrator: coordination, data flow, track_tasks
├── phases/
...
### Pattern 2: track_tasks Attachment/Collapse

```
Phase starts:
  → Sub-tasks ATTACHED to track_tasks (in_progress + pending)
...
Workflow preferences (auto mode, force explore, etc.) MUST be collected via ask_user in SKILL.md **before** dispatching to phases. Phases reference these as `workflowPreferences.{key}` context variables.
...
When one phase needs to invoke another phase within the same skill, read and execute the phase document directly. Do NOT use invoke_skill() routing back through SKILL.md.
...
|------------|--------|------------------|
| Flag parsing (`$ARGUMENTS.includes(...)`) | Preferences collected in SKILL.md | SKILL.md via ask_user |
...
| Skill routing for inter-phase (`invoke_skill(skill="...")`) | Use direct phase read | Direct `read_file("phases/...")` |

### Pattern 9: Compact Recovery (Phase Persistence)

Multi-phase workflows span long conversations. Context compression (compact) will naturally summarize earlier phase documents. The strategy uses **双重保险**: track_tasks 跟踪 active phase 保护其不被压缩，sentinel 作为兜底在压缩发生时触发恢复。

**Design principle**: track_tasks `in_progress` = active phase → protect from compact | Sentinel = re-read fallback if protection fails.

**Double insurance mechanism**:

| Layer | Role | Mechanism |
|-------|------|-----------|
| **1. Active Phase Protection** | 预防 | track_tasks `in_progress` 标记当前 phase → compact 时保留该 phase 完整内容，不压缩 |
...
#### Layer 1: Active Phase Protection (track_tasks 联动)

track_tasks 已经跟踪每个 phase 的执行状态。利用此信息驱动 compact 策略：

```
track_tasks status → Compact behavior:
...
Phase 1: Requirements Analysis    ← track_tasks tracks status
   └─ Ref: phases/01-xxx.md
Phase 2: Execution Engine         ← track_tasks tracks status
   └─ Ref: phases/02-xxx.md
...
```

**track_tasks 状态转换** 时自动更新 compact 保护范围:
```
...
> Recovery: `read_file("phases/0N-xxx.md")`
```
...
|-------|----------|---------|---------|
| 1 | phases/01-xxx.md | Planning | track_tasks 驱动 |
| 2 | phases/02-xxx.md | Execution | track_tasks 驱动 + 🔄 sentinel |

**Compact Rules**:
1. **track_tasks `in_progress`** → 保留完整内容，禁止压缩
2. **track_tasks `completed`** → 可压缩为摘要
3. **🔄 sentinel fallback** → 带此标记的 phase 包含 compact sentinel；若 compact 后仅存 sentinel 而无完整 Step 协议，**必须立即 `read_file("phases/0N-xxx.md")` 恢复后再继续**
```
...
> 1. This phase is track_tasks `in_progress` (active phase protection)
> 2. Full protocol (Step N.X — N.{M}) is in active memory, not just sentinel
> If only sentinel remains → `read_file("phases/0N-xxx.md")` now.
```

#### Handoff 注释

```javascript
// Phase N is tracked by track_tasks — active phase protection applies.
// Sentinel fallback: if compressed despite protection, re-read triggers automatically.
read_file("phases/0N-xxx.md")
```
...
| `allowed-tools` | `allowed-tools` | Expand wildcards: `invoke_skill(*)` → `Skill`, add commonly needed tools |
...
| track_tasks Pattern | Attachment/collapse lifecycle | Command track_tasks sections |
...
Collect workflow preferences via ask_user before dispatching to phases:
{ask_user code with preference derivation → workflowPreferences}
...
| Phase | Document | Purpose | Compact |
|-------|----------|---------|---------|
| 1 | [phases/01-xxx.md](phases/01-xxx.md) | ... | track_tasks 驱动 |
| N | [phases/0N-xxx.md](phases/0N-xxx.md) | ... | track_tasks 驱动 + 🔄 sentinel |
...

**Compact Rules**:
1. **track_tasks `in_progress`** → 保留完整内容，禁止压缩
2. **track_tasks `completed`** → 可压缩为摘要
3. **🔄 sentinel fallback** → 带此标记的 phase 包含 compact sentinel；若 compact 后仅存 sentinel 而无完整 Step 协议，必须立即 `read_file()` 恢复
...
- **track_tasks**: Mark Phase N completed, Phase N+1 in_progress
...
| What's the track_tasks granularity? | track_tasks Pattern | Some phases have sub-tasks, others are atomic |
...
| Does it need preference collection? | Interactive Preference Collection | Collect via ask_user in SKILL.md, pass as workflowPreferences |
| Does phase N hand off to phase M? | Direct Phase Handoff (Pattern 7) | Read phase doc directly, not invoke_skill() routing |
