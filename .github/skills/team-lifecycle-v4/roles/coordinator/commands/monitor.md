...
4.5. **evaluateSpecialistInjection** (based on detected codebase characteristics):
   - If callback from analyst, planner, or executor role:
     ...
     d. P0 matches → create_task with blockedBy on current stage, blocks downstream
     e. P1 matches → create_task parallel with REVIEW/TEST stage
...
5. Check for checkpoints:
   - CHECKPOINT-* with verdict "block" -> ask_user: Override / Revise upstream / Abort
...
3. For each IMPL task: create_task with structured description (dispatch.md template)
4. Set blockedBy:
...
3. send_message to supervisor (see SKILL.md Supervisor Wake Template):
   ...
   ```
4. Do NOT update_task in_progress — supervisor claims the task itself
