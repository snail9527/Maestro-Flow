...
- Responsibility: Analyze task -> Create team -> Dispatch tasks -> monitor_process progress -> Report results
...
3. Single session -> reconcile:
   a. Audit list_tasks, reset in_progress->pending
...
2. Clarify if ambiguous (ask_user: scope, deliverables, constraints)
...
4. Create tasks via create_task with blockedBy
