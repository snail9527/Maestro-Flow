...
- **Responsibility**: Validate session -> Reconcile state -> Create team -> Dispatch team-worker agents -> monitor_process progress -> Completion action -> Report results

## Boundaries

### MUST
- Validate session structure before any execution
- Reconcile session state with list_tasks on startup
...
- monitor_process progress via worker callbacks and route messages
...
### Step 3: Reconcile with list_tasks

Compare list_tasks() with session.completed_tasks, bidirectional sync.
...
For each task in task-analysis, check if exists in list_tasks, create if missing.
...
create_team if team does not exist.
...
| Mode | Behavior |
|------|----------|
| `interactive` | ask_user with Archive/Keep/Export options |
