  ...
  'maestro-analyze': [
    ...
    },
    {
      ...
      steps: [
        ...
        { name: '输出生成', description: '生成 Phase 级 analysis.md 和 context.md，直接供 plan 消费', detail: '下游 plan 步骤直接消费进入规划' },
      ],
    },
  ],
  'maestro-roadmap': [
    {
      ...
      steps: [
        ...
        { name: '持久化', description: '写入 roadmap.json 并更新 state.json 的里程碑/阶段结构', detail: '后续通过 analyze 步骤对各 Phase 深入分析' },
      ],
    },
  ],
  'maestro-plan': [
    {
      ...
      steps: [
        ...
        { name: 'Confirm 确认', description: '向用户展示计划摘要，等待批准后进入执行（-y 自动批准）', detail: '计划确认后进入 execute 步骤执行' },
      ],
    },
  ],
  'maestro-execute': [
