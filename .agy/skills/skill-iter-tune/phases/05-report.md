...
> Recovery: `view_file("phases/05-report.md")`
...
```javascript
const state = JSON.parse(view_file(`${state.work_dir}/iteration-state.json`));
...
```javascript
write_to_file(`${state.work_dir}/final-report.md`, report);
