...
Workflow preferences (auto mode, force explore, etc.) MUST be collected via ask_question in SKILL.md **before** dispatching to phases. Phases reference these as `workflowPreferences.{key}` context variables.
...
> Recovery: `view_file("phases/0N-xxx.md")`
```
...
3. **🔄 sentinel fallback** → 带此标记的 phase 包含 compact sentinel；若 compact 后仅存 sentinel 而无完整 Step 协议，**必须立即 `view_file("phases/0N-xxx.md")` 恢复后再继续**
```
...
> If only sentinel remains → `view_file("phases/0N-xxx.md")` now.
```
...
Collect workflow preferences via ask_question before dispatching to phases:
{ask_question code with preference derivation → workflowPreferences}
...
1. **TodoWrite `in_progress`** → 保留完整内容，禁止压缩
2. **TodoWrite `completed`** → 可压缩为摘要
3. **🔄 sentinel fallback** → 带此标记的 phase 包含 compact sentinel；若 compact 后仅存 sentinel 而无完整 Step 协议，必须立即 `view_file()` 恢复
