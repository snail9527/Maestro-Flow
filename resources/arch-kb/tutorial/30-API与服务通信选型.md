...
> 一句话点题:**API 的核心不是在 REST 和 gRPC 之间做格式选择,而是边界选择。同步还是异步、内部还是外部、强契约还是灵活查询、一次请求还是持续流,这些才决定你该怎么通信。**
...
> **架构判断:**先定交互语义,再定协议。不要先说要用 gRPC,而要先判断这条链路是否必须同步知道结果。
...
比如抢票系统([案例 01 StarArena](../cases/stararena-ticketing/README.md)):能否进入等候室要同步;出票通知可以异步;排队位置变化适合流式或轮询。
